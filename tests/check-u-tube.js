...
const constants = ["KAPPA", "MIN_INNER_RADIUS_MM", "BORE_RANGE"];
...
// 값 줄이기: 높이 ≥ 너비/2, 관 두께 ≤ 너비/2 − 안쪽 최소 반지름, 높이 차 ≤ 높이 − 너비/2, 액체는 반원 끝 ~ 관 꼭대기
{
  ...
  assert.deepStrictEqual(ok, {width: 20, height: 60, bore: 4, diff: 15, closedLeft: true, levelLeft: 30, levelRight: 40},
  ...
  assert.strictEqual(lib.clampOptions({...base, diff: 55}).diff, 50, "short arm still reaches the bend top");
  ...
  assert.strictEqual(lib.clampOptions({...base, levelLeft: 50}).levelLeft, 45, "closed arm caps the left level at height − diff");
  ...
  assert.strictEqual(same.levelLeft, 45, "same level follows the left value, capped by the lower arm");
  assert.strictEqual(same.levelRight, 45, "same level copies to the right");
}
...
// J자관: 윤곽은 열린 패스 하나. 오른쪽 위에서 시작해 바깥 반원, 막힌 왼쪽 위 뚜껑, 안쪽 반원을 지나 오른쪽 안쪽 위에서 끝난다
{
  ...
  nearPoint(o[4].anchor, [0, 70], "left cap outer corner sits diff below the top");
  nearPoint(o[5].anchor, [6, 70], "left cap inner corner");
  nearPoint(o[6].anchor, [6, 20], "inner arc start");
  nearPoint(o[8].anchor, [34, 20], "inner arc end");
  nearPoint(o[9].anchor, [34, 100], "ends at the right inner top");
  assert.ok(isCorner(o[4]) && isCorner(o[5]), "cap corners have no handles");
...
assert.ok(source.includes('if (p[0] !== "v2" || p.length < 13) return;'), "settings tag bumped for the two new fields");
