...
assert.ok(source.indexOf('var SETTINGS_TAG = "v1"') < source.indexOf("applySavedSettings();"),
  "SETTINGS_TAG must be declared before applySavedSettings() runs");
assert.ok(source.indexOf("var SETTINGS_LENGTH = 55") < source.indexOf("applySavedSettings();"),
...
function side(count, siblings, parentMark) {
  return {
    ...
    siblings: siblings || [person(0), person(0), person(0)],
  };
}
...
function spec(overrides) {
  return Object.assign({
    ...
    markRatio: 0.8,
    sides: [side(0), side(0)],
    child: {kind: 0, gender: 0, pheno: 0, mark: 0},
  }, overrides);
}
...
// 1. 형제 없음 + 물음표: 조부모가 부모 바로 위에 오고, 외가 쪽이 밀려 부부선이 길어진다. 번호는 1~6
{
  ...
  assert.deepStrictEqual(layout.nodes.map((node) => node.number).sort((a, b) => a - b), [1, 2, 3, 4, 5, 6], "six numbered members");
  ...
  assert.ok(layout.question !== null, "question mark present");
  close(layout.question.x, childX, "question mark under the couple line midpoint");
  close(layout.question.y, -60, "question mark on the third generation");
...
{
  const siblings = [person(0), person(1), person(0)];
  const s = spec({sides: [side(2, siblings), side(0)], child: {kind: 2, gender: 1, pheno: 0, mark: 0}});
  const layout = layoutPedigree(s);
  const uncle = nodeOf(layout, siblings[0]);
  const aunt = nodeOf(layout, siblings[1]);
  const father = nodeOf(layout, s.sides[0].parent);
  assert.strictEqual(nodeOf(layout, siblings[2]), undefined, "third sibling is not drawn when count is 2");
  ...
  const child = nodeOf(layout, s.child);
  assert.strictEqual(layout.question, null, "no question mark when a child is drawn");
...
// 3. 원문자로 가린 구성원은 번호를 건너뛰고, 선은 작은 원 가장자리에서 끝난다
{
  ...
  assert.deepStrictEqual(layout.nodes.map((node) => node.number).sort((a, b) => a - b), [0, 1, 2, 3, 4, 5], "numbering skips the hidden member");
  hasLine(layout, [father.x + 5, -30, mother.x - 4, -30], "couple line ends at the mark circle edge");
}

// 4. 양쪽 형제가 많아 조부모 부부가 서로 멀면 벌리지 않는다
{
  const s = spec({sides: [side(3), side(3)]});
  const layout = layoutPedigree(s);
  const patMid = (0 + 42) / 2;
  const patGF = nodeOf(layout, s.sides[0].grand[0]);
