...
(function() {
    ...
    var optionRow1 = pnlOptions.add("group");   // 두 줄로 나눠 폭을 줄인다
    ...
    var chkPreview = optionRow2.add("checkbox", undefined, "미리보기");
    ...
    function addSlider(parent, labelText, minValue, maxValue, initialValue, unit, step) {
        ...
        input.characters = 5;
        var slider = row.add("scrollbar", undefined, initialValue, minValue, maxValue);
    ...
    var sldColumns = addSlider(pnlGeometry, "가로 육각형", 1, 10, 4, "", 1);
    var sldRows = addSlider(pnlGeometry, "세로 육각형", 1, 8, 3, "", 1);
    var sldLayers = addSlider(pnlGeometry, "적층 수", 1, 8, 3, "", 1);
    var sldBond = addSlider(pnlGeometry, "C-C 결합 길이", 2, 15, 6, "mm", 0.1);
    var sldLayerGap = addSlider(pnlGeometry, "층간 거리", 3, 35, 14, "mm", 0.1);
    var sldAtom = addSlider(pnlGeometry, "탄소 구 지름", 1, 12, 4, "mm", 0.1);
    var sldBrightness = addSlider(pnlGeometry, "탄소 밝기", 40, 160, 100, "%", 1);

    var pnlView = win.add("panel", undefined, "관찰 각도");
    pnlView.orientation = "column";
    pnlView.alignChildren = "left";
    pnlView.spacing = 2;
    var sldAngleR = addSlider(pnlView, "오른쪽 각도", 91, 179, 132, "°", 1);
    var sldAngleL = addSlider(pnlView, "왼쪽 각도", 91, 179, 108, "°", 1);
