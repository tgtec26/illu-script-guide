...
(function() {
    ...
    function makeEnergyEngine() {
        return makeFormEngine({
            ...
            controls: [
                ...
                {key: "count", label: "지점 수", unit: "개", min: 3, max: 7, step: 1, value: 5},
                {key: "height", label: "높이", unit: "mm", min: 20, max: 150, step: 1, value: 50},
                {key: "width", label: "너비", unit: "mm", min: 3, max: 40, step: 0.5, value: 8},
                {key: "space", label: "막대 간격", unit: "mm", min: 0, max: 30, step: 0.5, value: 5},
                {panel: "표시"},
                {key: "balls", check: "낙하 물체", value: true},
                {key: "legend", check: "범례", value: true},
                {key: "names", check: "지점 이름", value: true},
                {key: "total", check: "역학적 에너지 선", value: false},
                {key: "font", label: "글자 크기", unit: "pt", min: 5, max: 20, step: 0.5, value: 8}
            ],
            draw: drawEnergy
        });
    ...
    // 지점 i의 위치 에너지 비율 (맨 위 1 → 바닥 0)
    function energyFractions(count) {
        var list = [];
        for (var i = 0; i < count; i++) list.push(1 - i / (count - 1));
        return list;
    }

    function drawEnergy(t, o) {
        var mm = t.mm;
        var H = o.height * mm, W = o.width * mm;
        var f = energyFractions(o.count);
        var PE_K = 50, KE_K = 10;
        var letters = "ABCDEFG";
        var x0 = 0;

        if (o.balls) {
            // 낙하 경로: 점선 위에 공, 공 옆에 지점 이름, 아래는 지면
            var bx = -8 * mm;
            t.line([bx, H], [bx, 0], 0.3, 100, [1.5, 1.5]);
            for (var i = 0; i < f.length; i++) {
                t.circle(bx, f[i] * H, 1.5 * mm, 40, 100, 0.3);
                if (o.names) t.text(letters.charAt(i), bx - 4 * mm, f[i] * H, o.font);
            }
            t.line([bx - 2 * mm, 0], [bx + 4 * mm, 0], 0.5);
        }

        if (o.kind === 0) {
            for (var j = 0; j < f.length; j++) {
                var left = x0 + j * (W + o.space * mm);
                var pe = f[j] * H;
                if (pe > 0) t.rect(left, pe, left + W, 0, PE_K, 100, 0.3);
                if (pe < H) t.rect(left, H, left + W, pe, KE_K, 100, 0.3);
                if (o.names) t.text(letters.charAt(j), left + W / 2, -o.font * 0.7, o.font);
            }
            var right = x0 + f.length * W + (f.length - 1) * o.space * mm;
            t.line([x0 - 2 * mm, 0], [right + 2 * mm, 0], 0.5);
            if (o.total) t.line([x0 - 2 * mm, H], [right + 2 * mm, H], 0.5, 100, [2, 1.5]);
        } else {
            var TW = W * 3;
            t.path([[x0, 0], [x0, H], [x0 + TW, H]], true, PE_K, 100, 0.3);
            t.path([[x0, 0], [x0 + TW, 0], [x0 + TW, H]], true, KE_K, 100, 0.3);
            for (var k = 1; k < f.length - 1; k++) t.line([x0, f[k] * H], [x0 + TW, f[k] * H], 0.3, 100, [1.5, 1.5]);
            if (o.total) t.line([x0 - 2 * mm, H], [x0 + TW + 2 * mm, H], 0.5, 100, [2, 1.5]);
            if (!o.legend) {
                t.text("위치", x0 + TW * 0.3, H * 0.72, o.font, "center", 0);
                t.text("운동", x0 + TW * 0.7, H * 0.28, o.font);
            }
        }

        if (o.legend) {
            var lx = (o.kind === 0 ? x0 + f.length * (W + o.space * mm) : x0 + W * 3 + 4 * mm) + 2 * mm;
            var box = o.font * 0.9;
            t.rect(lx, H, lx + box, H - box, PE_K, 100, 0.3);
            t.text("위치 에너지", lx + box + 1.5 * mm, H - box / 2, o.font, "left");
            t.rect(lx, H - box * 2, lx + box, H - box * 3, KE_K, 100, 0.3);
            t.text("운동 에너지", lx + box + 1.5 * mm, H - box * 2.5, o.font, "left");
        }
    }
