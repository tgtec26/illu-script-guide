const scripts = [
  ...
  { id: "particle-model", category: "도형", name: "입자 모형 (원자·분자·이온 결합)", file: "01_도형/Object_ParticleModel.jsx", summary: "원자 모형·분자 모형·이온 결합 모형 생성기를 한 창의 탭으로 묶은 스크립트입니다. 핵 전하량 표시, 전자 - 기호, 전자 껍질 선, 핵·전자 3D 조명 효과, 크기(1껍질 지름·핵 지름·전자 지름·핵 전하량 글자)는 세 탭이 같이 쓰고, 정원을 하나 선택하고 실행하면 그 원을 최외곽 껍질로 삼아 크기를 맞춥니다. [원자] H~Ar 원소를 다중 선택하고 이온 전하(-3~+3), 1번 껍질 수평 배열, 2·3번 껍질 22.5° 이동을 고릅니다. [분자] H₂·N₂·O₂·F₂·Cl₂·HCl·H₂O·CO₂·Cl₂O·NH₃·CH₄ 공유 결합 모형을 껍질 겹침(%)과 함께 그립니다. [이온 결합] LiF·NaF·NaCl·KCl·Na₂O·K₂O·MgO·CaO·MgCl₂·CaCl₂ 화합물을 이온 전하 글자 크기·이온 간격·1껍질 전자 3시·9시 옵션으로 그립니다. 실시간 미리보기를 지원합니다.", tags: ["원자", "분자", "이온", "전자", "껍질", "모형", "화학"] },
  { id: "graph-tools", category: "도형", name: "그래프·표 (축 눈금·마커·표·점선 분할선)", file: "01_도형/Object_GraphTools.jsx", summary: "축 눈금·그래프 마커·표·점선 분할선을 한 창의 탭으로 묶은 스크립트입니다. 탭마다 필요한 선택이 다르며(축 눈금: 사각형, 마커: 꺾은선 패스, 표·점선 분할선: 축에 나란한 사각형) 선택에 맞지 않는 탭은 흐리게 표시됩니다. [축 눈금] 사각형을 기준으로 아래축과 왼쪽축을 만들고, 화살표와 눈금 숫자를 배치합니다. [그래프 마커] 선택한 꺾은선 그래프의 모든 고정점에 원·사각형·삼각형 마커를 넣습니다. 크기(0.5~2mm), 채움(K 10 단위), 테두리(0 또는 0.3~1pt, 100K)를 미리보기로 조절합니다. [표] 선택한 사각형을 행·열로 나눈 표로 바꿉니다. 셀마다 테두리 사각형이 생겨 글자를 정렬하기 쉽습니다. [점선 분할선] 선택한 사각형을 행·열 수만큼 나누는 점선(0.3pt, 2pt 선·1pt 간격)을 넣습니다. 2행 2열이면 가운데 가로·세로 점선 하나씩으로 4칸이 됩니다. 슬라이더로 수를 정한 뒤 완료를 누르면 그립니다.", tags: ["그래프", "축", "눈금", "마커", "과학", "표", "정렬", "점선", "분할"] },
  ...
  { id: "round-solids", category: "도형", name: "원 입체 (원기둥·원뿔·구)", file: "01_도형/Object_RoundSolids.jsx", summary: "선택한 원을 기준으로 원기둥·원뿔·구를 만드는 세 스크립트를 한 창의 탭으로 묶은 스크립트입니다. 세 탭 모두 가로·세로가 같은 원 패스 하나를 선택해야 합니다. [원기둥] 선택한 원을 기준으로 원기둥을 세웁니다. 면별 K 농도 음영과 속이 비치는 관 구멍을 지원합니다. [원뿔] 선택한 원을 기준으로 원뿔 또는 원뿔대를 만듭니다. 밑면·윗면 지름, 높이 균등 분할선, 면별 K 농도, X·Y·Z축 시점을 조절합니다. [구] 선택한 원을 기준으로 구를 만들고 경도선과 위도선을 배치합니다. 경도선 회전과 X·Y·Z축 시점 조절을 지원합니다.", tags: ["입체", "원기둥", "원뿔", "원뿔대", "분할", "구", "위도", "경도"] },
  ...
  { id: "3dline", category: "도형", name: "3D → 2D 라인 (입체 도형·돌출·회전체)", file: "01_도형/Object_3DLine.jsx", summary: "입체 도형·돌출·회전체를 한 창의 탭으로 묶은 스크립트입니다. 시점, 숨은선(파선·실선·생략), 면 채우기(없음·단일 음영·광원 자동: 밝기·대비·광원 방위·높이), 위치 이동은 탭들이 같이 씁니다. [입체 도형1·2] 선택 없이 실행. 직육면체·정사면체·정팔면체·정십이면체·정이십면체·각기둥·각뿔·각뿔대·원기둥·원뿔·원뿔대·빨대(속 빈 원기둥)를 만듭니다. 1은 시점(가로 회전·위아래 기울기·화면 회전·커스텀 프리셋·원근)으로 물체를 돌리는 카메라 방식, 2는 결정 구조와 같은 관찰 각도(오른쪽·왼쪽 각도, 앞·뒤 면 거리, 등각·2등각·3등각·사면체·층상 프리셋)로 그림을 정하는 제도 방식입니다. [돌출] 패스를 하나 이상 선택. 패스를 앞뒤로 밀어 기둥을 만듭니다(닫힌 패스는 뚜껑 있는 기둥, 열린 패스는 띠, 여러 개면 겹친 안쪽이 구멍). [회전체] 단면 패스와 축 직선(앵커 2개)을 선택. 단면을 축 둘레로 돌린 회전체를 만듭니다(사각형에 접한 축이면 원기둥, 원과 떨어진 축이면 도넛). 돌출·회전체처럼 볼록하지 않은 도형도 광선 교차로 숨은선을 정확히 가르며, 선택에 맞지 않는 탭은 흐리게 표시됩니다. 확인하면 원본 패스·축은 지워집니다.", tags: ["입체", "3D", "숨은선", "음영", "정다면체", "원기둥", "원뿔", "돌출", "익스트루드", "회전체", "도넛", "파이프"] },
  ...
  { id: "electricity", category: "도형", name: "전기 (회로 기호·코일 도선·에너지 흐름)", file: "01_도형/Object_Electricity.jsx", summary: "회로 기호 삽입·코일 감긴 도선·에너지 흐름 화살표를 한 창의 탭으로 묶은 스크립트입니다. 탭마다 필요한 선택이 다르며(회로 기호: 앵커 2개 직선, 코일 도선: 사각형, 에너지 흐름: 사각형) 선택에 맞지 않는 탭은 흐리게 표시됩니다. [회로 기호] 직선을 선택하고 저항·직렬 전지·교류 전원·스위치·인덕터·축전기·전류계·전압계 기호를 클릭한 순서대로 선 위에 균등 배치합니다. [코일 도선] 선택한 사각형을 도선으로 바꾸고 나선 투영으로 코일을 감습니다. 감는 횟수에 따라 코일 간격이 달라집니다. [에너지 흐름] 선택한 사각형을 연료 에너지 상자로 삼아 그 위로 갈라져 올라가는 에너지 흐름 화살표를 그립니다. 상자 이름·값(숫자 뒤에 이탤릭 E 자동)을 넣고 2·3·4개로 나눈 뒤 구간 슬라이더나 값 입력으로 비율을 정하며, 항목마다 이름과 ?로 표시 여부를 고릅니다. 첫 화살표는 12시 방향으로 곧게, 마지막은 90° 꺾여 3시 방향으로 나가고 가운데 화살표는 각도를 조절합니다. 화살표마다 꺾임 높이·길이·꺾임 반지름·음영(K%), 공통으로 상자 폭·높이, 화살촉 길이·폭(몸통 폭 대비 %라 닮은꼴), 글자 크기, 값 높이, 위치를 조절합니다. 확인하면 원본 사각형은 지워집니다.", tags: ["회로", "물리", "기호", "과학", "전자기", "에너지", "화살표", "다이어그램"] },
  ...
  { id: "smooth-path", category: "도형", name: "패스 정리 (앵커 제거·부드럽게)", file: "01_도형/Object_SmoothPath_v2.jsx", summary: "패스를 정리합니다. 두 탭으로 나뉩니다. [앵커 제거] 원본 핸들을 그대로 두고, 빼도 허용 오차(mm) 안에 맞는 고정점만 삭제 — VectorFirstAid의 Super Smart Remove 방식. 수천 패스·수만 앵커를 다루므로 미리보기 없이 확인 때 한 번에 적용합니다. [부드럽게] 연필로 그린 패스용. '급한 곳만 다듬기'를 켜면 곡률 상위 N% 구간만 평활하고 나머지 앵커·핸들은 원본 그대로 둡니다. 앵커 줄이기(허용 오차 안에서 필요 없는 고정점 삭제)와 곡선 다듬기(꺾인 곳과 곡률이 튀는 곳을 펴서 부드럽게)를 0~100 강도로 각각 조절하고, 모서리 유지 각도보다 급하게 꺾인 점만 모서리로 남깁니다. 세포막처럼 크기가 중요한 그림이 쪼그라들지 않도록 평활은 면적을 지키는 방식입니다. 부드럽게 탭은 미리보기가 선택한 패스에 바로 적용되고, 취소하면 원래 좌표로 돌아갑니다.", tags: ["연필", "곡선", "앵커", "단순화", "세포막", "트레이스", "앵커 제거"] },
  { id: "cell-division", category: "도형", name: "염색체·세포 분열 (염색체·상동 염색체·감수 분열·세포 주기)", file: "01_도형/Object_CellDivision.jsx", summary: "염색체 모형·상동 염색체·감수 분열·세포 주기를 한 창의 탭으로 묶은 스크립트입니다. 상동 염색체와 감수 분열은 그림이 들어갈 사각형 하나를 선택해야 하며, 선택에 맞지 않는 탭은 흐리게 표시됩니다. [염색체 모형] 핵형 분석 설명용 염색체 모형을 최대 4개까지 한 번에 그립니다. 탭마다 염색 분체 수(1·2개)·전체 길이·두께·중심절 위치와 지름·벌림 각도·p암/q암 휨·내부 음영을 따로 정하고, 켜진 염색체가 선택 개체(없으면 아트보드) 중심에 가로로 나란히 놓입니다. 암은 중심절 쪽이 좁고 끝이 둥근 형태, 중심절은 흰 원, 외곽선 0.3pt입니다. [상동 염색체] 선택한 사각형에 맞춰 상동 염색체 한 쌍을 그립니다. 좌우 두께·중심절 위치와 지름·염색체 간격을 조절하고 유전자 좌를 최대 3개까지 표시합니다. 중심절 지름을 바꾸면 p암·q암이 따라 붙습니다. [감수 분열] 선택한 사각형을 비계로 감수 분열 과정(G1기 → 중기 1 → 중기 2 → 딸세포 → 정자)을 그립니다. 줄마다 세로 간격, 딸세포 사이 가로 간격, 단계별 지름, 정자 머리·꼬리 모양과 회전, 전체 위치를 조절하면 중기 2 위치와 화살표 각도가 따라옵니다. [세포 주기] 세포 주기 도넛 다이어그램을 만듭니다. 외경·내경, 네 구간의 비율과 제목, 안쪽을 도는 화살표를 조절합니다.", tags: ["과학", "생물", "핵형", "다이어그램"] },
  ...
  { id: "crystal-structure", category: "도형", name: "결정 구조 (입방정계·다이아몬드·흑연)", file: "01_도형/Object_CrystalStructure.jsx", summary: "입방정계 단위세포·다이아몬드 결정·흑연 결정 생성기를 한 창의 탭으로 묶은 스크립트입니다. 구 3D 조명 효과, 구 외곽선, 색상 표현(컬러·회색 음영), 관찰 각도(오른쪽·왼쪽 각도, 앞·뒤 면 거리, Isometric·Dimetric·Trimetric·Tetrahedral·Layered 프리셋), 미리보기 위치 이동은 탭들이 같이 씁니다. [입방정계1·2] 단순·체심·면심 입방 등 격자 유형을 다중 선택해 1셀 또는 2×2×2로 그리고 라인·밀집·절단 표현, 숨은선, 구 지름·밝기·셀 간격을 조절합니다. 1은 관찰 각도(모서리 각도로 정하는 제도 방식), 2는 3D → 2D 라인과 같은 시점(가로 회전·위아래 기울기·화면 회전, 정면·등각·측면·윗면 프리셋)으로 투영합니다. [다이아몬드] 1셀·8셀·피라미드 클러스터로 그리고 단위세포 라인, C-C 결합선, 경계 결합 완성, 숨은선, 탄소 구 지름·결합선 굵기·밝기를 조절합니다. [흑연] AB·AA 적층으로 육각형 가로·세로 수, 적층 수, C-C 결합 길이, 층간 거리, 층간 점선, 탄소 구 지름·밝기를 조절합니다.", tags: ["결정", "단위세포", "입방정계", "다이아몬드", "흑연", "격자", "3D"] },
  { id: "mechanics", category: "도형", name: "역학 (진자·수평 던지기·사인 곡선·코일 스프링)", file: "01_도형/Object_Mechanics.jsx", summary: "진자 운동·수평 던지기 포물선·사인 곡선·코일 스프링을 한 창의 탭으로 묶은 스크립트입니다. 탭마다 필요한 선택이 다르며(진자: 수평선, 수평 던지기: 없음, 사인 곡선: 패스 하나, 코일 스프링: 원 하나) 선택에 맞지 않는 탭은 흐리게 표시됩니다. [진자 운동] 선택한 수평선을 천장 삼아 진자 그림을 그립니다. 각도 θ, 진자 길이, 궤적, 직각 표시를 넣습니다. [수평 던지기] 높은 곳에서 오른쪽으로 수평으로 던진 물체가 지면에 닿을 때까지의 포물선 궤적을 화면 중앙에 그립니다. 높이(m)·수평 속도(m/s)를 넣으면 낙하 시간과 수평 도달거리를 계산해 보여 주고, 도면 축척은 실제 1m = 2mm(가로·세로 동일), 중력 9.8m/s², 공기 저항은 없습니다. 선 두께와 위치를 미리보기로 조절하며 값을 바꿔도 투사 지점은 움직이지 않습니다. [사인 곡선] 선택한 패스를 축으로 삼아 sin 곡선을 만듭니다. 직선은 그대로, 곡선·원은 그 곡선을 따라 파형이 감깁니다. 진폭·파장·축 이동·선 두께를 조절합니다. [코일 스프링] 선택한 원을 기준으로 코일 스프링을 만듭니다. 좌우 폭, 위아래 높이, 감는 횟수를 조절합니다.", tags: ["과학", "역학", "포물선", "운동", "파동", "입체", "스프링"] },
  ...
  { id: "dna-model", category: "도형", name: "DNA 모형 (평면 모형·염기 서열)", file: "01_도형/Object_DnaModel.jsx", summary: "DNA 평면 모형과 DNA·RNA 염기 서열을 한 창의 탭으로 묶은 스크립트입니다. [평면 모형] 염기서열을 입력하면 인산·당·염기 사다리 모양의 DNA 평면 모형을 그립니다. 1·2가닥, 가로·세로, 염기 모양·크기·음영, 범례를 조절합니다. [염기 서열] DNA 위쪽 가닥 서열을 입력하면 골격선·연결선·염기 글자로 DNA 1·2와 RNA 세 줄을 그립니다. 가릴 서열을 지정해 ?·ⓐ·㉠·Ⅰ 박스로 덮고, 선 굵기·연결선 길이·염기 간격·줄 간격을 미리보기로 조절합니다.", tags: ["과학", "생명", "유전"] },
