const scripts = [
  ...
  { id: "3dline", category: "도형", name: "3D → 2D 라인 (입체 도형·돌출·회전체)", file: "01_도형/Object_3DLine.jsx", summary: "입체 도형·돌출·회전체 세 가지를 한 창의 탭으로 묶은 스크립트입니다. 시점(가로 회전·위아래 기울기·화면 회전·커스텀 프리셋·원근), 숨은선(파선·실선·생략), 면 채우기(없음·단일 음영·광원 자동: 밝기·대비·광원 방위·높이), 위치 이동은 세 탭이 같이 씁니다. [입체 도형] 선택 없이 실행. 직육면체·정사면체·정팔면체·정십이면체·정이십면체·각기둥·각뿔·각뿔대·원기둥·원뿔·원뿔대·빨대(속 빈 원기둥)를 만듭니다. [돌출] 패스를 하나 이상 선택. 패스를 앞뒤로 밀어 기둥을 만듭니다(닫힌 패스는 뚜껑 있는 기둥, 열린 패스는 띠, 여러 개면 겹친 안쪽이 구멍). [회전체] 단면 패스와 축 직선(앵커 2개)을 선택. 단면을 축 둘레로 돌린 회전체를 만듭니다(사각형에 접한 축이면 원기둥, 원과 떨어진 축이면 도넛). 돌출·회전체처럼 볼록하지 않은 도형도 광선 교차로 숨은선을 정확히 가르며, 선택에 맞지 않는 탭은 흐리게 표시됩니다. 확인하면 원본 패스·축은 지워집니다.", tags: ["입체", "3D", "숨은선", "음영", "정다면체", "원기둥", "원뿔", "돌출", "익스트루드", "회전체", "도넛", "파이프"] },
