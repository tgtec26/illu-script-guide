const scripts = [
  ...
  { id: "smooth-path", category: "도형", name: "곡선 부드럽게 정리", file: "01_도형/Object_SmoothPath.jsx", summary: "연필로 그린 패스를 정리합니다. 앵커 제거(원본 핸들을 그대로 두고, 빼도 허용 오차 안에 맞는 고정점만 삭제 — VectorFirstAid의 Super Smart Remove 방식, 이미지 트레이스 결과에 적합), 앵커 줄이기(허용 오차 안에서 필요 없는 고정점 삭제)와 곡선 다듬기(꺾인 곳과 곡률이 튀는 곳을 펴서 부드럽게)를 0~100 강도로 각각 조절하고, 모서리 유지 각도보다 급하게 꺾인 점만 모서리로 남깁니다. 세포막처럼 크기가 중요한 그림이 쪼그라들지 않도록 평활은 면적을 지키는 방식입니다. 미리보기가 선택한 패스에 바로 적용되고, 취소하면 원래 좌표로 돌아갑니다.", tags: ["연필", "곡선", "앵커", "단순화", "세포막", "트레이스", "앵커 제거"] },
