const scripts = [
  ...
  },
  {
    ...
    summary: "축 눈금·그래프 마커·표·점선 분할선·모델 곡선·태양 스펙트럼을 한 창의 탭으로 묶은 스크립트입니다. 각 탭의 코드와 저장 키는 원래 스크립트 그대로입니다.",
    requires: "탭마다 필요한 선택이 다릅니다. 선택에 맞지 않는 탭은 흐리게 표시되고 툴팁에 이유가 적힙니다. 태양 스펙트럼 탭은 선택 없이도 열립니다.",
    tags: ["그래프", "축", "눈금", "마커", "과학", "표", "정렬", "점선", "분할", "곡선", "정규분포", "생장", "하디바인베르크", "생명", "화학", "태양", "복사", "스펙트럼", "지구과학", "대기"],
    tabs: [
      { id: "axis-ticks", name: "축 눈금", summary: "사각형을 기준으로 아래축과 왼쪽축을 만들고, 화살표와 눈금 숫자를 배치합니다.", requires: "사각형 하나를 선택합니다." },
      { id: "markers", name: "그래프 마커", summary: "선택한 꺾은선 그래프의 모든 고정점에 원·사각형·삼각형 마커를 넣습니다. 크기(0.5~2mm), 채움(K 10 단위), 테두리(0 또는 0.3~1pt, 100K)를 미리보기로 조절합니다.", requires: "꺾은선 그래프(패스)를 선택합니다." },
      { id: "table", name: "표", summary: "선택한 사각형을 행·열로 나눈 표로 바꿉니다. 셀마다 테두리 사각형이 생겨 글자를 정렬하기 쉽습니다.", requires: "가로·세로 변이 축에 나란한 사각형 하나를 선택합니다." },
      { id: "dashed-grid", name: "점선 분할선", summary: "선택한 사각형을 행·열 수만큼 나누는 점선(0.3pt, 2pt 선·1pt 간격)을 넣습니다. 2행 2열이면 가운데 가로·세로 점선 하나씩으로 4칸이 됩니다. 슬라이더로 수를 정한 뒤 완료를 누르면 그립니다.", requires: "가로·세로 변이 축에 나란한 사각형 하나를 선택합니다." },
      { id: "model-curves", name: "모델 곡선", summary: "선택한 사각형을 그래프 영역으로 삼아 과학의 이상적 모델 곡선을 그립니다. 정규분포, 이론적 생장 곡선(J형), 실제 생장 곡선(S형·로지스틱), 하디-바인베르크(p²·2pq·q²), 효소 반응 속도(미카엘리스-멘텐), 산소 해리 곡선(힐 식), 생존 곡선(Ⅰ·Ⅱ·Ⅲ형), 활성화 에너지 도표(촉매 곡선), 지수 감소(반감기), 맥스웰-볼츠만 분포, 반비례(보일 법칙), 거듭제곱 12종입니다. 사각형 너비가 x, 높이가 y 범위이고 종류마다 봉우리 위치·폭·가파름·환경 수용력·높이 같은 옵션과 K·Vmax 점선을 미리보기로 조절합니다. 사각형은 기본으로 남겨 두어 곡선을 그린 뒤 같은 사각형으로 축 눈금을 이어 만들 수 있습니다.", requires: "가로·세로 변이 축에 나란한 사각형 하나를 선택합니다." },
      { id: "solar-spectrum", name: "태양 스펙트럼", summary: "대기 밖과 지표면에서의 태양 복사 에너지 그래프를 그립니다. NREL ASTM G173 표준 스펙트럼(대기 밖·지표면 AM1.5)을 내장하고, 단순화(가우시안 평활) 슬라이더로 교과서 수준까지 요철을 줄입니다. 오존(자외선)·수증기·CO₂(적외선) 흡수 영역을 닫힌 면으로 만들고, 축·0.5 µm 눈금·축 범례·자외선/가시광선/적외선 화살표를 함께 넣습니다. 흡수 영역 위쪽은 대기 밖 곡선까지(교과서식) 또는 포락선까지 중 고릅니다.", requires: "선택 없이 실행하면 화면 가운데에 그려지고 위치 패널로 옮깁니다. 사각형 하나를 선택하면 그 사각형이 그래프 영역이 되고 확인할 때 지워집니다." }
    ]
  },
