const scripts = [
  ...
  { id: "earth-layers", category: "도형", name: "지구 내부 구조", file: "01_도형/Object_EarthLayers.jsx", summary: "화면 가운데에 지각·맨틀·외핵·내핵 단면을 그립니다. 경계는 실제 깊이 비율(2900 km, 5100 km, 중심 6400 km)을 따르고, 너무 얇은 지각은 반지름에 대한 %로 두께를 과장합니다. 창 맨 위 탭으로 방식을 고릅니다. 평면은 단면 각도로 정하는 부채꼴이며, 360°(원)·180°(반원)·90°(1/4)는 라디오 단추로 고르고 그 밖의 각은 각도(°) 행에 적습니다. 3D 절단은 시점(기울기·돌리기)에서 본 입체로, 절개한 구(절단 각도만큼 쐐기를 잘라낸 구, 180°면 반구 단면)와 잘라낸 조각(쐐기 조각만, 위쪽 반만 선택)을 고르고 절개 방향을 돌립니다. 반구 분리는 지각·맨틀·외핵을 안쪽 층이 빠져 속이 파인 반구 껍질로, 내핵을 입체 음영이 있는 구로 그리고, 손잡이 4개짜리 슬라이더(또는 숫자 칸)로 층마다 가로 위치(mm)를 따로 정하며 단면 폭(%)을 조절합니다. 글자는 한글은 Spoqa, 숫자·영문은 GSMediumB1으로 나뉩니다. 층마다 음영(K)을 넣고, 45° 방향 각 층 가운데에서 오른쪽으로 지시선을 뽑아 이름(원하면 고체·액체 상태도)을 붙이며, 평면에서는 오른쪽 가로 반지름 아래에 깊이(0, 2900, 5100, 6400 km)를 적습니다. 이름이 가까우면 세로로 벌려 겹치지 않게 합니다. 반지름·지시선 길이·글자 크기를 조절하며 미리보기를 보면서 옮길 수 있습니다.", requires: "선택 없이 실행합니다.", tags: ["과학", "지구과학", "중학교", "지권", "지구 내부", "지각", "맨틀", "외핵", "내핵"] },
